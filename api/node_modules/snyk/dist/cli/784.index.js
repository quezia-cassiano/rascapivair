...
async function monitor(...args0) {
    ...
    for (const path of paths) {
        ...
        try {
            ...
            const unsupportedPackageManagers = [{ label: 'Swift PM', name: 'swift' }];
...
async function resolveAndTestFacts(ecosystem, scans, options) {
    ...
    return featureFlagEnabled || (unmanagedDepsOverride === null || unmanagedDepsOverride === void 0 ? void 0 : unmanagedDepsOverride.toLowerCase()) === 'true'
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.formatUnmanagedResults = exports.selectAndExecuteTestStrategy = exports.testEcosystem = void 0;
const config_1 = __webpack_require__(25425);
const is_ci_1 = __webpack_require__(10090);
const promise_1 = __webpack_require__(90430);
...
async function testEcosystem(ecosystem, paths, options) {
    ...
    spinner_1.spinner.clearAll();
    if (common_2.isUnmanagedEcosystem(ecosystem) && options['print-graph']) {
        const [target] = paths;
...
async function getUnmanagedDepGraph(scans) {
    var _a, _b;
    const results = [];
    const orgId = (_a = (await getSelf())) === null || _a === void 0 ? void 0 : _a.data.attributes.default_org_context;
    for (const [, scanResults] of Object.entries(scans)) {
        for (const scanResult of scanResults) {
            const taskId = await resolve_test_facts_1.submitHashes({ hashes: (_b = scanResult === null || scanResult === void 0 ? void 0 : scanResult.facts[0]) === null || _b === void 0 ? void 0 : _b.data }, orgId);
...
async function buildDepGraph(root, manifestFilePath, lockfilePath, lockfileVersion, options) {
    ...
    switch (lockfileVersion) {
        ...
        case snyk_nodejs_lockfile_parser_1.NodeLockfileVersion.YarnLockV2:
            return await lockFileParser.parseYarnLockV2Project(manifestFileContents, lockFileContents, options);
        case snyk_nodejs_lockfile_parser_1.NodeLockfileVersion.NpmLockV2:
        case snyk_nodejs_lockfile_parser_1.NodeLockfileVersion.NpmLockV3:
...
async function getIssues(issuesRequestAttributes, orgId) {
    const payload = {
        method: 'POST',
        url: `${config_1.default.API_HIDDEN_URL}/orgs/${orgId}/unmanaged_ecosystem/issues?version=2022-06-29~experimental`,
        body: issuesRequestAttributes,
    };
    return await promise_1.makeRequestRest(payload);
}
exports.getIssues = getIssues;
async function getDepGraph(id, orgId) {
    const payload = {
        method: 'GET',
        url: `${config_1.default.API_HIDDEN_URL}/orgs/${orgId}/unmanaged_ecosystem/depgraphs/${id}?version=2022-05-23~experimental`,
    };
    return await promise_1.makeRequestRest(payload);
}
...
async function createDepGraph(hashes, orgId) {
    const payload = {
        method: 'POST',
        url: `${config_1.default.API_HIDDEN_URL}/orgs/${orgId}/unmanaged_ecosystem/depgraphs?version=2022-05-23~experimental`,
        body: hashes,
    };
    return await promise_1.makeRequestRest(payload);
}
...
async function sendAndParseResults(payloads, spinnerLbl, root, options) {
    ...
    for (const originalPayload of payloads) {
        const request = async () => {
            let step = 0;
            let error;
            while (step < common_2.RETRY_ATTEMPTS) {
                debug(`sendTestPayload retry step ${step} out of ${common_2.RETRY_ATTEMPTS}`);
                try {
                    ...
                    if (err instanceof errors_1.InternalServerError ||
                        ...
                        await common_1.sleep(common_2.RETRY_DELAY);
                    }
                    else {
                        break;
                    }
